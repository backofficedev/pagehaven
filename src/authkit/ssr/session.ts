...
export async function decryptSession(
	...
	try {
		const sessionString = Buffer.from(sealedSession, "base64").toString(
			"utf-8",
		);
...
export async function saveSession(session: AuthkitSession): Promise<void> {
	...
	const cookieName =
		getConfig<string>("cookieName", "wos-session") ?? "wos-session";
	const cookieMaxAge =
		getConfig<number>("cookieMaxAge", 400 * 24 * 60 * 60) ?? 400 * 24 * 60 * 60;
...
export async function withAuth(): Promise<{
	user: User;
	accessToken: string;
	impersonator?: AuthkitSession["impersonator"];
} | null> {
	...
	const { authenticated, session: updatedSession } =
...
export function getAuthorizationUrl(options?: {
	...
	const state = options?.returnPathname
		? Buffer.from(
				JSON.stringify({ returnPathname: options.returnPathname }),
			).toString("base64")
		: undefined;
